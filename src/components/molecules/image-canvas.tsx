...
export const ImageCanvas = ({
  ...
  const [controlTriggered, setControlTriggered] = useState(false);
  const [boundings, setBoundings] = useState<TBoundings[]>([]);
  const [highlightBorder, setHighlightBorder] = useState<number>();
  ...
  const [isMovingBounding, setIsMovingBounding] = useState<boolean>(false);
  ...
  const handleMouseUp = (
    ...
    if (boundingTriggered !== undefined) {
      setControlTriggered(false);
      setBoundingTriggered(undefined);
      setIsMovingBounding(false);
  ...
  useEffect(() => {
    ...
    if (isMouseClick && !isMovingBounding && boundingTriggered === undefined) {
      ctx.strokeRect(
        mouseStart[0],
        mouseStart[1],
    ...
    boundings.forEach((item, itemIndex) => {
      ...
      if (
        ...
        trigger = highlightBorder === itemIndex;
      }
      ...
      const boundingWidth = item.x2 - item.x1;
      const boundingHeight = item.y2 - item.y1;

      if (
        mousex > centerBounding[0] - 20 &&
        mousex < centerBounding[0] + 20 &&
        mousey > centerBounding[1] - 20 &&
        mousey < centerBounding[1] + 20
      ) {
        if (boundingSizeBuffer === undefined)
          setBoundingSizeBuffer([boundingWidth, boundingHeight, itemIndex]);

        if (isMouseClick) {
          setBoundingTriggered(itemIndex);
          setIsMovingBounding(true);
        }
        ...
        if (
          ...
          isMovingBounding &&
          boundingSizeBuffer &&
          boundingSizeBuffer[2] == itemIndex
        ) {
      ...
      posts.filter((num, index) => {
        if (
          ...
          trigger = true;
          controlTrigger = index;
          setControlTriggered(true);
          if (isMouseClick) {
            setBoundingTriggered(itemIndex);
          }

          if (isMouseClick && boundingTriggered === itemIndex && index === 0) {
